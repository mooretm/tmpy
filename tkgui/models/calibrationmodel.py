...
class CalibrationModel:
    ...
    def __init__(self, settings):
        logger.debug("Initializing CalibrationModel")
        # Assign variables
    ...
    def get_cal_file(self):
        ...
        logger.debug("Importing calibration file")
    ...
    def calc_offset(self):
        ...
        logger.debug("Calculating SLM offset")
        ...
        logger.debug("Starting level (dB FS): %s",
                     self.settings['cal_level_dB'].get()
                    )
        logger.debug("SLM reading (dB): %s",
                     self.settings['slm_reading'].get()
                     )
        logger.debug("SLM offset: %s",
    ...
    def calc_level(self, desired_level_dB):
        ...
        # Provide feedback
        logger.debug("Desired level in dB: %f",
                     self.settings['desired_level_dB'].get()
                    )
        logger.debug("SLM offset: %f",
                     self.settings['slm_offset'].get()
                    )
        logger.debug("Adjusted level (dB): %f",
